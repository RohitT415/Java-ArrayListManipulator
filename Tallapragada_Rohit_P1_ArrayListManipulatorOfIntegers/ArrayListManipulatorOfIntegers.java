...
public class ArrayListManipulatorOfIntegers
{
    ...
    {
        long sum = 0; // ... correct this statement
        // ... sum the elements in ArrayList nums
    ...
    {
        long product = 0; // ... correct this statement
        // ... generate the product of the elements in ArrayList numbs
        for (int i = 0; i < numbs.size(); i++)
        {
            product *= (long)numbs.get(i);
        }
        return product;
    }   
    ...
    public void removeOdds(ArrayList<Integer> seriesOfNumbers)
    {
        ...
        for (int i = seriesOfNumbers.size(); i >= 0; i--)
        {
            if (seriesOfNumbers.get(i) % 2 != 0)
            {
    ...
    public void removeAllButOneNeighboringDuplicates(ArrayList<Integer> quantities)
    {
        ...
        {
            while (quantities.get(i) == quantities.get(i + 1))
            {
                quantities.remove(i);
            }
        }
    ...
    collectionOfNumbers)
    {
        // ... finish the code to match the description of the method.
        // ... This can be tricky.
        boolean contingous = true;
        int count = 0;
        int arrayLength = collectionOfNumbers.size();
        int increment = 0;
        while (contingous)
        {
            count = 0;
            contingous = true;
            while (count + 1 < arrayLength)
            {
                while (count + 1 < arrayLength && (collectionOfNumbers.get(count) 
                    == collectionOfNumbers.get(count + 1)))
                {
                    increment = collectionOfNumbers.get(count);
                    collectionOfNumbers.remove(count);
                    arrayLength--;
                    while ((count < arrayLength) && (increment == collectionOfNumbers.get(count)))
                    {
                        collectionOfNumbers.remove(count);
                        arrayLength--;
                        contingous = true;
